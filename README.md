...
the bird’s rarity.
	6.	Some birds “flee” randomly or when barely missed — a short flight animation, then reappear elsewhere in
...
its own z-index slot between occlusion layers).
	•	Render order example: background → occlusionLayer[0] → birds(z:1) → occlusionLayer[1] → birds(z:2) → ...
...
for sharpness at max zoom.
	•	Bird animation: 1–3 frame sprite cycle via CSS/JS interval — simple to validate
user-uploaded content.
	•	Bird state machine: spawning (fade-in) → visible → [fleeing → hidden → reappear] → visible → clicked/despawned.
